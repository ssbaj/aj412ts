...
invisible(readline(prompt="Press [enter] to continue"))
cat(" 			 ", '\n')
cat(" summary(Arima(gdp_sa2,order=c(1,1,0),include.drift=T,method='CSS-ML') ) ", '\n')
cat(" summary(Arima(gdp_sa2, order=c(0,1,1), seasonal=c(0,0,2),include.drift=T, method='CSS-ML')) ", '\n')
...
cat(" ## 차분인 2회 이상 발생하면 - seasonal difference 포함 - drift가 포함하지 않는다 ", '\n')
cat(" 			 ", '\n')
cat(" auto.arima(gdp_sa2, method='CSS-ML') ", '\n')
cat(" auto.arima( gdp_sa2, seasonal=TRUE, optim.method='BFGS') ", '\n')
...
cat(" R1<-Arima( (gdp_sa2), order=c(1,1,1), method='CSS-ML')  ", '\n')
...
cat(" R3 <- auto.arima( gdp_sa2, seasonal=FALSE, method='CSS-ML' )  ", '\n')
cat(" 			 ", '\n')
...
cat("      include.mean=T, xreg=dummy, method='CSS-ML')  ", '\n')
cat(" Areport(R4)  ", '\n')
...
cat("      xreg=dummy, include.mean=T, method='CSS-ML' )  ", '\n')
cat(" Areport(R5)  ", '\n')
cat(" # --------------------  ", '\n')
