...
if (base::missing(lagsx)) { lags<-lags } else { lags<-lagsx }
  ...
  barplot(r, col='steelblue', ylim=c(-1, 1), border=NA, xlab='Lags', ylab="ACF", names.arg=1:lags, main=c('Number of Data', length(xdatas)) )
  box()
  abline(h=0)
