...
cat("  VARselect(df[,c(2:4)], lag.max=8, type='const')  # SC기준으로 lag는 2  ", '\n')
cat("  .....  ", '\n')
cat("# 1차 차분하기 ----------------      ", '\n')
cat("  df<-mkd(df, 2) ", '\n')
cat("  df<-mkdlog(df, 3) ", '\n')
...
cat("  Re <- VAR(df[-c(1) ,c(5:6)], exogen=df[,c(7)], type='const', p=2) # p=2대신 lag=4를 사용하면 자동으로 Lag결정", '\n')
cat("  roots(Re) # stable because all roots <1 ", '\n')
cat("       ", '\n')
cat("# 그랜저 인과관계 분석II ---------------  ", '\n')
cat("  causality(Re, cause='d_ip') # d_ip does not cause d_m1", '\n')
cat("  causality(Re, cause='d_m1') ", '\n')
cat("  causality(Re, cause='d_tb') ", '\n')
cat("       ", '\n')
cat("# 충격반응함수(Impulse Response Function)------------- ", '\n')
cat("  IRF.ip1<-irf(Re, impulse='d_m1', response='d_ip', ortho=T, n.ahead=30, boot=TRUE )   ", '\n')
cat("  plot(IRF.ip1, ylab='ip반응', main='m1 Shock')   ", '\n')
cat("  IRF.ip2<-irf(Re, impulse='d_tb', response='d_ip', ortho=T, n.ahead=30, boot=TRUE )   ", '\n')
cat("  plot(IRF.ip1, ylab='ip반응', main='tb Shock')   ", '\n')
cat("  IRF.m11<-irf(Re, impulse='d_ip', response='d_m1', ortho=T, n.ahead=30, boot=TRUE )   ", '\n')
cat("  plot(IRF.m11, ylab='m1반응', main='ip Shock')     ", '\n')
cat("  IRF.m12<-irf(Re, impulse='d_tb', response='d_m1', ortho=T, n.ahead=30, boot=TRUE )   ", '\n')
cat("  plot(IRF.m12, ylab='m1반응', main='tb Shock')     ", '\n')
cat("  IRF.tb1<-irf(Re, impulse='d_ip', response='d_tb', ortho=T, n.ahead=30, boot=TRUE )   ", '\n')
cat("  plot(IRF.tb1, ylab='tb반응', main='ip Shock')     ", '\n')
cat("  IRF.tb2<-irf(Re, impulse='d_m1', response='d_tb', ortho=T, n.ahead=30, boot=TRUE )   ", '\n')
