...
cat(" gdp <- statSearch(stat_code='200Y004', item_code1='1400', cycle='Q') # 분기별 GDP   ", '\n')
...
cat(" hp_gdp <- cbind( gdp$DATE, hp.gdp$trend, hp.gdp$cycle) ", '\n')
...
cat("  ", '\n')
cat(" # 연이율로 변경한 잠재성장률 ", '\n')
cat(" hp_gdp$pc_annual <- ( (1+hp_gdp$pc)^4 - 1)*100   ", '\n')
cat("  ", '\n')
cat(" # 경기변동 비지니스 사이틀 그래프 ", '\n')
cat(" xtscycle <- xts(hp_gdp$cycle, order.by=hp_gdp$DATE)   ", '\n')
cat(" plot(xtscycle, main = '월별자료', col = 'blue')   ", '\n')
cat("  ", '\n')
cat(" # 잠재성장률 그래프 ", '\n')
cat(" xtspc_annual <- xts(hp_gdp$pc_annual, order.by=hp_gdp$DATE) ", '\n')
