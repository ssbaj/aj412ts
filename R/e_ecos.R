...
cat('\014')
cat("    ", '\n')
cat("\033[1;31m------- OOOO연도가 몇 번째 행부터 시작하는지 찾는 코드 ------- \033[0m ", '\n')
cat("\033[1;32m# DataSet에서 날짜는 DATE변수에 기록되어 있음 \033[0m ", '\n')
cat(" Adata<-상해지수 ", '\n')
cat(" start_year <- '2020'  ", '\n')
cat(" n<-nrow(Adata)  ", '\n')
cat(" for(i in 1:n){   ", '\n')
cat(" tmp_time <- as.character( substring( Adata$DATE[i], 1, 4 ) )   ", '\n')
cat(" if(tmp_time == start_year) {   ", '\n')
cat(" nx<-i  ", '\n')
cat(" cat( start_year,'년 최초 데이터는 ',nx,'행부터 시작', sep='' )  ", '\n')
cat(" break } ", '\n')
cat(" } ", '\n')
cat("  ", '\n')
}}
